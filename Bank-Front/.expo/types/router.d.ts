...
export namespace ExpoRouter {
  type StaticRoutes = `/` | `/(app)/` | `/(app)/(tabs)` | `/(auth)/` | `/(auth)/(tabs)` | `/(tabs)` | `/AddChat` | `/_sitemap` | `/chatGroup` | `/context/ctx` | `/context/useStorageState` | `/deposit` | `/modal` | `/operations` | `/singup` | `/transfer` | `/withdrawal`;
  type DynamicRoutes<T extends string> = `/${SingleRoutePart<T>}`;
  type DynamicRouteTemplate = `/[chatGroup]`;
